...
void GetReading(){
   ...
   aDcData0[ulMeasCount] = analogRead(32);  // Read ADC7
   aDcData1[ulMeasCount] = analogRead(33);  // Read ADC7
   aDcData2[ulMeasCount] = analogRead(34);  // Read ADC7
   aDcData3[ulMeasCount] = analogRead(35);  // Read ADC7  
   }
...
clearArray();
  ...
  timerAlarmWrite(timer, 20000, true);  // 20 milliseconds
...
for(int loOp=32;loOp<36;loOp++){
  ...
  if(loOp==33) { y = aDcData1[1] / 30; dIspV=aDcData1[ulMeasCount];}
  if(loOp==34) { y = aDcData2[1] / 30; dIspV=aDcData2[ulMeasCount];}
  if(loOp==35) { y = aDcData3[1] / 30; dIspV=aDcData3[ulMeasCount];}
  ...
  for (int x = 1; x < ulMeasCount; x+=1) {
    ...
    if(loOp==33)  y2 = aDcData1[record] / 30;
    if(loOp==34)  y2 = aDcData2[record] / 30;
    if(loOp==35)  y2 = aDcData3[record] / 30;
